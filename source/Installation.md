...
In order to build the local PostgreSQL database you will need to get your computer set up. There are a few ways to install this locally. 
...
### Installing Conda
We installed Anaconda last semester. You can download it from the website, but you can also install it through Homebrew. 
...
Python works best in small development environments. Conda can help you with this. To create a new environment:

```zsh
conda create -n mimic python=3.13.9 
```
...
[psycopg2](https://pypi.org/project/psycopg2/) — Required by SQLAlchemy to access PostgreSQL databases
...
conda install pandas sqlalchemy matplotlib seaborn jupyter
conda install -c conda-forge psycopg2
```

### Installing PostgreSQL@17
I've installed and am running PostgreSQL@17. It's probably best if we all use the same version if possible, though different computers may have different CPU architecture which may or may not be supported with version 17. 
...
This will write the line `export PATH="/opt/homebrew/opt/postgresql@17/bin:$PATH"` to the file `.zshrc`. Your terminal will now find this version of PostgreSQL making the following commands function. By default brew doesn't automatically add PostgreSQL@17 to the `PATH`. It gets a bit more technical, but this will work. 
...
This information is from the [MIT-LCP](https://github.com/MIT-LCP/mimic-code/tree/main/mimic-iv/buildmimic/postgres) GitHub repository, which is a curated resource of ways to deal with the MIMIC series of datasets, including the Google online repo that we've been discussing. 


If you run the following code (assuming you have MIMIC-IV access) it will download the data, create a new PostgreSQL database, create the tables/schema, and then add all the information. It then links all the tables together (remember foreign keys and 1:many relationships?), and then indexes the whole thing.
...
It will ask for your password after this line. It's your password for physionet.
